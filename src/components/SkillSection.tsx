...
const languageItems = ["HTML", "CSS", "JavaSript", "TypeScript","Python","GO"]
const frontendItems = ['React','Vue','Next.js', 'Vite']
const backendItems = ["Node.js"]
const mobileItems = ['Flutter']
const devItems = ['React','Vue','Next.js', 'Vite', 'Node.js','Flutter']
const projectItems = ['Git',"Jira","Bitbucket"]
const designItems = ['Figma','Adobe XD', 'Adobe Illustrator', 'Adobe Photoshop', 'Adobe InDesign', 'Affinity']
...
const SkillSection = () => {
  ...
  return (
    <div className='w-full grid'>
      ...
      <span className='chosen-font text-center text-xl border-b-[1px] p-2 mb-1 border-b-white/10'>Skills</span>
      <button className='absolute top-2 p-0 bg-transparent' onClick={()=>setIsCollapsed((prev)=>!prev)}>
        <ChevronDown className={`${isCollapsed && '-rotate-90'} transition-transform duration-300`} />
      </button>
      </div>
    ...
    {isCollapsed ? (
      <div className='flex gap-2 items-center justify-center'>
        ...
        <button className='p-[0.6em]' onClick={()=>setIsCollapsed(false)}>... more</button>
      </div>
      ):(
      <>
